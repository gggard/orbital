...
def test_sample_metrics_stores_usage_and_restarts(reconciler):
    ...
    with (
        patch("orbital.k8s.reconciler.metrics.fetch_app_usage", return_value=sample),
...
def test_sample_metrics_skips_non_running_apps(reconciler):
    ...
    with (
        patch("orbital.k8s.reconciler.metrics.fetch_app_usage") as usage,
        patch("orbital.k8s.reconciler.metrics.fetch_pod_restarts") as restarts,
    ):
        reconciler._sample_metrics([app])
...
def test_sample_metrics_usage_failure_does_not_block_restarts(reconciler):
    ...
    with (
        patch("orbital.k8s.reconciler.metrics.fetch_app_usage", side_effect=RuntimeError("boom")),
        patch("orbital.k8s.reconciler.metrics.fetch_pod_restarts", return_value=info),
    ):
...
def test_sample_metrics_none_results_store_nothing(reconciler):
    app = make_app()
    with (
        patch("orbital.k8s.reconciler.metrics.fetch_app_usage", return_value=None),
        patch("orbital.k8s.reconciler.metrics.fetch_pod_restarts", return_value=None),
    ):
        reconciler._sample_metrics([app])

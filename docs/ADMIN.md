...
By default, build pods push/pull the in-cluster registry over plain HTTP
(`registry.tls.enabled=false`) — this is the minikube/dev quick-start path
and needs no setup. On any cluster where the pod network isn't otherwise
segmented, that means built app images travel unencrypted and the registry's
identity isn't verified (a compromised pod on the same network could snoop
or tamper with pushes/pulls). Two ways to bound that risk, and they're
complementary, not exclusive:

1. **Enable registry TLS** (`registry.tls.enabled: true`). With
   `registry.internal.enabled: true` (the chart's own dev/demo registry),
   the chart generates a self-signed CA and a server certificate for the
   registry Service's in-cluster DNS name at `helm install`/`upgrade` time
   (via Sprig's `genCA`/`genSignedCert`), stores them in a Secret
   (`<release>-registry-tls`), and reuses that Secret across upgrades (via
   `lookup`) so the cert doesn't rotate and break trust on every release.
   The registry container picks it up via `REGISTRY_HTTP_TLS_CERTIFICATE`/
   `REGISTRY_HTTP_TLS_KEY`; the control plane and build Jobs get the CA
   cert's PEM text plumbed in as `ORBITAL_REGISTRY_CA_CERT`
   (`registry_ca_cert` / `registry_tls_enabled` in `config.py`), and
   `buildkitd.toml` is generated with a `ca=[...]` trust entry instead of
   `http = true`.

   **Bringing your own registry** (`registry.internal.enabled: false`, e.g.
   Harbor/ECR/GCR reachable in-cluster): the chart has no CA of its own to
   generate for a registry it doesn't run, so set
   `registry.tls.existingSecret` to a Secret you create with keys
   `tls.crt`/`tls.key`/`ca.crt` — the control plane and build Jobs trust its
   `ca.crt` the same way.

   **Tradeoff**: this is a self-signed CA trusted only by this chart's own
   components (control plane, build Jobs) via the explicit `ca=[...]`/
   `ORBITAL_REGISTRY_CA_CERT` plumbing above — it is *not* added to any pod's
   OS root trust store, so nothing outside those two paths will trust it.
   That's the right tradeoff for registry traffic that never leaves the
   cluster; it would not be appropriate for a registry meant to be reachable
   (and trusted) from outside.

2. **NetworkPolicy** (tracked separately, issue #79): restrict which
   namespaces/pods can reach the registry Service at all, regardless of
   whether TLS is enabled. This is the recommended baseline even when TLS is
   on (defense in depth) and the minimum recommended mitigation if you keep
   the default plaintext path.

**First-install bootstrap note**: the post-install Jobs that build/push the
platform's base images (`baseImages.build`/`baseImages.buildStatic`, both
`true` by default) also need the registry's CA trust when
`registry.tls.enabled=true`. That CA is generated by a normal chart resource
in the *same* install, but Helm's `lookup` can only see cluster state from
*before* the current `helm install`/`upgrade` began — so on a **brand-new**
install with both `registry.tls.enabled=true` and the base-image builds
enabled, those Jobs fail fast with an explicit error the first time. Either
run `helm upgrade` again right after (the Secret exists by then, and these
hooks re-run on every install/upgrade) to build and push the base images, or
set `baseImages.build=false`/`baseImages.buildStatic=false` on that first
install to build them out-of-band instead. This only affects the very first
install with both settings on; normal upgrades are unaffected, and the guard
doesn't fire at all when both `baseImages.build` and `baseImages.buildStatic`
are `false`.

# Specification: Orbital — Self-Hosted Streamlit Hosting Platform on Kubernetes
...
This document specifies Orbital, a self-hosted platform for deploying, running, and managing [Streamlit](https://streamlit.io) applications on a Kubernetes cluster. A developer points the platform at a git repository, and minutes later the app is live on its own subdomain, with secrets management, logs, automatic redeploys on push, and hibernation of idle apps.

### Goals

- **Frictionless developer experience**: deploy from a git repo in a few clicks, manage apps from a web dashboard.
...
- **No custom Linux (apt) packages.** Only Python packages installable with pip/uv into the shared base image are supported. Apps needing system libraries beyond those in the base image are out of scope.
...
- **FR-2.3** If the changed commit does not touch the dependency file (§4.3) or the Python version, the build reuses the cached dependency layer — only the code layer is rebuilt (fast path). Dependency changes trigger a full dependency install.
- **FR-2.4** Redeploy triggers are rate-limited per app (default: max 5 per minute); excess triggers are coalesced into one trailing redeploy.

### 4.3 Dependency resolution

- **FR-3.1** The platform searches for exactly one Python dependency file, first in the main file's directory, then in the repository root. Detection priority:
...
- **FR-3.3** `packages.txt` is **detected and rejected with a warning**: the build proceeds, the log states that Linux packages are not supported on this platform.
...
- **FR-5.6 Resource indicators**: the dashboard shows per-app memory/CPU usage vs. limits and flags apps that were OOM-killed or restarted.

### 4.6 Sharing & viewer access
...
- **FR-6.5** No limit on the number of private apps.
...
- **FR-8.3** Waking does not require authentication beyond what the app's sharing mode requires.

### 4.9 Locked runtime configuration
...
- **FR-9.1** The platform mounts a `config.toml` that overrides app-supplied config for:
  ...
  - `browser.gatherUsageStats = false` (self-hosted default is telemetry off)
- **FR-9.2** All other options from the repo's `.streamlit/config.toml` (repo root only) are honored — theming in particular.
...
flowchart LR
    ...
    subgraph cluster [Kubernetes cluster]
        subgraph platformns [namespace: orbital-platform]
...
Naming/labeling conventions: all per-app objects are named `app-<app-id>` and labeled `app.orbital.io/id=<app-id>`, `app.orbital.io/managed-by=control-plane`. The reconciler only touches objects carrying the managed-by label.

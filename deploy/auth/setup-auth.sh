...
# auth settings to .env. Reads SH_APPS_DOMAIN / SH_APPS_URL_PORT from .env.
set -euo pipefail
cd "$(dirname "$0")/../.."

PROFILE="${PROFILE:-streamlit-host}"
DOMAIN=$(grep '^SH_APPS_DOMAIN=' .env | cut -d= -f2)
PORT=$(grep '^SH_APPS_URL_PORT=' .env | cut -d= -f2 || true)
PORT_SUFFIX=""
[ -n "$PORT" ] && [ "$PORT" != "80" ] && PORT_SUFFIX=":$PORT"

CLIENT_SECRET="${CLIENT_SECRET:-streamlit-host-dev-secret}"
...
kubectl --context "$PROFILE" -n streamlit-platform rollout status deploy/oauth2-proxy --timeout=300s
kubectl --context "$PROFILE" -n streamlit-platform rollout status deploy/keycloak --timeout=600s

MINIKUBE_IP=$(minikube -p "$PROFILE" ip)
GATEWAY_IP=$(echo "$MINIKUBE_IP" | sed 's/\.[0-9]*$/.1/')

grep -q '^SH_AUTH_ENABLED=' .env || cat >> .env <<EOF
SH_AUTH_ENABLED=true
SH_OAUTH2_PROXY_AUTH_URL=http://${MINIKUBE_IP}:30480/oauth2/auth
SH_AUTHZ_BASE_URL=http://${GATEWAY_IP}:8000
EOF
